...
int main() {
  ...
  h.onMessage([&current_lane, &target_velocity,&map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy](
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); ++i) {
            ...
            if (d < (2+4*current_lane+2) && d > (2+4*current_lane-2)) {
              ...
              check_car_s+=(static_cast<double>(previous_path_size)*.02*check_speed);
              if ((check_car_s > car_s) && (check_car_s-car_s) < 30) {
                //means there is a car in our lane ahead and less then 30 m away!
                //slow tf down
                //target_velocity = 30; //mph
                too_close = true;
              }
            }
          }
